...
class AddressData extends AbstractFixture implements ContainerAwareInterface, OrderedFixtureInterface
{
	...
	public function load(ObjectManager $manager)
	{
		...
		$this->createAddressFull($manager, "address_news", "Campus", "", "", "14460", "Colombelles");
...
//		$manager->persist($tb2Address);
        ...
        private function createAddressFull($manager, $ref, $address1, $address2, $address3, $postalCode, $city)
        {
            $Address_test = new Address();
            $Address_test->setAddress1($address1);
            $Address_test->setAddress2($address2);
            $Address_test->setAddress3($address3);
            $Address_test->setPostalCode($postalCode);
            $Address_test->setCity($city);
            $this->addReference($ref, $Address_test);
            $manager->persist($Address_test);
        }
}